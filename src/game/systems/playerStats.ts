...
function clampTotalStats(stats: PlayerStatSet): PlayerStatSet {
  return {
    ...
    // hitChance: capped 0–1; base 0.75 ensures player lands most attacks
    hitChance: Math.min(1, Math.max(0, stats.hitChance)),
    critChance: Math.min(1, Math.max(0, stats.critChance)),
    // critMultiplier floor of 1.0 prevents crits from doing less than normal hits
    critMultiplier: Math.max(1, stats.critMultiplier),
    dodgeChance: Math.min(1, Math.max(0, stats.dodgeChance)),
